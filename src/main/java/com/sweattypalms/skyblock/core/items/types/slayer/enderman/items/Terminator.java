...
public class Terminator extends SkyblockItem implements IHasAbility, IShortBow {
    ...
    public static class TerminatorShortBow extends AbilityManager.SHORT_BOW {
        ...
        public void apply(Event event) {
            ...
            ArmorStand as = player.getWorld().spawn(player.getEyeLocation().add(player.getEyeLocation().getDirection().multiply(1.5)), ArmorStand.class);
            as.setVisible(false);
            MozangStuff.noHit(as);
            as.setMarker(true);
            as.setSmall(true);
    ...
    public static org.bukkit.util.Vector rotateAroundY(Vector vector, double angle) {
    ...
    public static class TerminatorSalvation implements ITriggerableAbility, IUsageCost {
        ...
        private ArmorStand createInvisibleArmorStand(Player p) {
            ArmorStand as = p.getWorld().spawn(p.getEyeLocation(), ArmorStand.class);
            as.setVisible(false);
            MozangStuff.noHit(as);
            as.setMarker(true);
            as.setSmall(true);
        ...
        private void moveArmorStandForward(ArmorStand armorstand) {
            ...
            ParticleEffect.DRIP_LAVA.display(0, 0, 0, 0, 4, armorstand.getLocation(), 10);
            ParticleEffect.REDSTONE.display(0, 0, 0, 0, 4, armorstand.getLocation(), 10);

        }

        private boolean isArmorStandBlocked(ArmorStand armorstand) {
            Block block = armorstand.getTargetBlock((HashSet<Material>) null, 1);
            return block != null && !block.getType().isTransparent();
        }
